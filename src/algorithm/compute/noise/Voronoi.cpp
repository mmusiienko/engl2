...
namespace EnGl
{
	void VoronoiNoise2D::Fill(AssetHandle<Texture2D> textureA, const VoronoiNoiseParams& params)
	{
		static AssetHandle<ComputeShader> shaderHandle = AssetManager::Load<ComputeShader>(AssetManager::COMPUTE_SHADER_DIR / "VORONOI2D");
		...
		auto points = GeneratePointsWithinBounds(texture->Properties(), params);
		Texture2D pointsTex{ static_cast<u32>(points.size()), 1, {.Data = (void*)points.data(), .CpuFormat = GL_RG, .GpuFormat = GL_RG32F} };

		f32 cellW = static_cast<f32>(texture->Properties().w) / params.NPoints;
		f32 cellH = static_cast<f32>(texture->Properties().h) / params.NPoints;
	...
	std::vector<glm::vec2> VoronoiNoise2D::GeneratePointsWithinBounds(const Texture2D::Props& props, const VoronoiNoiseParams& params)
	{
		auto idx = [params](u32 i, u32 j) { return i * (params.NPoints + 2) + j; };

		std::vector<glm::vec2> res((params.NPoints + 2) * (params.NPoints + 2));
		Random rand;
		f32 cellW = static_cast<f32>(props.w) / params.NPoints;
		f32 cellH = static_cast<f32>(props.h) / params.NPoints;
		...
		for (u32 i = 0; i <= params.NPoints + 1; i++)
		{
			for (u32 j = 0; j <= params.NPoints + 1; j++)
			{
				...
				f32 dx = 0, dy = 0;
				if (i == 0) dx = -static_cast<f32>(props.w);
				if (i == params.NPoints + 1) dx = static_cast<f32>(props.w);
				if (j == 0) dy = -static_cast<f32>(props.h);
				if (j == params.NPoints + 1) dy = static_cast<f32>(props.h);
	...
	void VoronoiNoise3D::Fill(AssetHandle<Texture3D> textureA, const VoronoiNoiseParams& params)
	{
		static AssetHandle<ComputeShader> shaderHandle = AssetManager::Load<ComputeShader>(AssetManager::COMPUTE_SHADER_DIR / "VORONOI3D");
		...
		f32 cellW = static_cast<f32>(texture->Properties().w) / params.NPoints;
		f32 cellH = static_cast<f32>(texture->Properties().h) / params.NPoints;
		f32 cellD = static_cast<f32>(texture->Properties().d) / params.NPoints;
	...
	std::vector<glm::vec4> VoronoiNoise3D::GeneratePointsWithinBounds(const Texture3D::Props& props, const VoronoiNoiseParams& params)
	{
		...
		f32 cellW = static_cast<f32>(props.w) / params.NPoints;
		f32 cellH = static_cast<f32>(props.h) / params.NPoints;
		f32 cellD = static_cast<f32>(props.d) / params.NPoints;
		...
		for (u32 i = 0; i <= params.NPoints + 1; i++)
		{
			for (u32 j = 0; j <= params.NPoints + 1; j++)
			{
				for (u32 k = 0; k <= params.NPoints + 1; k++)
				{
					...
					if (j == 0) dy = -static_cast<f32>(props.h);
					if (j == params.NPoints + 1) dy = static_cast<f32>(props.h);
					if (k == 0) dz = -static_cast<f32>(props.d);
					if (k == params.NPoints + 1) dz = static_cast<f32>(props.d);
