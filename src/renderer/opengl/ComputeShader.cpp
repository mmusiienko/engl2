...
namespace EnGl
{
	...
	void ComputeShader::BindReadTexture(const Texture& tex, u32 unit)
	{
		GL_CHECK(glBindImageTexture(unit, tex.Id(), 0, GL_FALSE, 0, GL_READ_ONLY, tex.Properties().GpuFormat));
	}

	void ComputeShader::BindWriteTexture(const Texture& tex, u32 unit)
	{
		GL_CHECK(glBindImageTexture(unit, tex.Id(), 0, GL_FALSE, 0, GL_WRITE_ONLY, tex.Properties().GpuFormat));
	}

	void ComputeShader::BindReadWriteTexture(const Texture& tex, u32 unit)
	{
